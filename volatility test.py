...
#This creates the volatility test in Figure 5.14.
...
P_E = np.zeros((3,6))
D_E = np.zeros((3,6))
reg_P_E = np.zeros((3,6))
reg_D_E = np.zeros((3,6))
...
for v in range(1,7):
    ...
    P_E[:,v-1] = Price_error
    D_E[:,v-1] = Delta_error
    reg_P_E[:,v-1] = reg_Price_error
    reg_D_E[:,v-1] = reg_Delta_error
...
fig=plt.figure(0)
plt.plot(x,P_E[0,:],'.-',color = 'black')
plt.plot(x,P_E[1,:],'.-',color = 'dimgray')
plt.plot(x,P_E[2,:],'.-',color = 'gainsboro')
plt.plot(x,reg_P_E[0,:],'.-',color = 'darkred')
plt.plot(x,reg_P_E[1,:],'.-',color = 'red')
plt.plot(x,reg_P_E[2,:],'.-',color = 'salmon')

black_patch = mpatches.Patch(color='black', label='SP 1st. order')
dimgray_patch = mpatches.Patch(color='dimgray', label='SP 2nd. order')
gainsboro_patch = mpatches.Patch(color='gainsboro', label='SP 3rd. order')

darkred_patch = mpatches.Patch(color='darkred', label='Poly 3rd. order')
red_patch = mpatches.Patch(color='red', label='Poly 6th. order')
salmon_patch = mpatches.Patch(color='salmon', label='Poly 9th. order')
plt.title('Option Price Error ')
plt.xlabel('Volatility')
plt.ylabel('Error')
plt.legend(handles=[black_patch,dimgray_patch,gainsboro_patch, darkred_patch, red_patch, salmon_patch])
plt.grid(b=True, which='major', color='#666666', linestyle='-')
fig=plt.figure(1)
plt.plot(x,D_E[0,:],'.-',color = 'black')
plt.plot(x,D_E[1,:],'.-',color = 'dimgray')
plt.plot(x,D_E[2,:],'.-',color = 'gainsboro')
plt.plot(x,reg_D_E[0,:],'.-',color = 'darkred')
plt.plot(x,reg_D_E[1,:],'.-',color = 'red')
plt.plot(x,reg_D_E[2,:],'.-',color = 'salmon')
